...
def define_cool_grammar(print_grammar=False):
    ...
    element %= (
        ...
        lambda h, s: CallNode(*s[3], obj=s[1]),
    )
    element %= (
        element + at + idx + dot + func_call,
        lambda h, s: CallNode(*s[5], obj=s[1], at_type=s[3]),
    )
    element %= func_call, lambda h, s: CallNode(*s[3])
    ...
    atom %= (
        true,
        lambda h, s: BooleanTrueNode(s[1]),
    )  # verificar #include en el tokenizer
    atom %= false, lambda h, s: BooleanFalseNode(s[1])  # include en el tokenizer
    atom %= string, lambda h, s: StringNode(s[1])  # include en el tokenizer
