...
def first_error(compiler_output: list, errors: list):
    ...
    assert (
        line == oline and column == ocolumn and error_type == oerror_type
...
def first_error_only_line(compiler_output: list, errors: list):
    ...
    assert line == oline and error_type == oerror_type, UNEXPECTED_ERROR % (
        error_type,
        line,
        column,
        oerror_type,
...
def compare_errors(
    ...
    try:
        if platform.system() == "Windows":
            sp = subprocess.run(
                ["python", compiler_path, cool_file_path],
                capture_output=True,
                timeout=timeout,
            )
        else:
            sp = subprocess.run(
                ["bash", compiler_path, cool_file_path],
                capture_output=True,
                timeout=timeout,
            )
