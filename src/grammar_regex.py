...
(
    ...
    CharRegex,
    Reserved,
) = regex_grammar.nonTerminals(
    *"Regex,ConcatenationRegex,ClosureRegex,ContentRegex,"
...
# (s)|(r)
Regex != Regex + alt + ConcatenationRegex / (ast.UnionNode, (0, 2)) | ConcatenationRegex

# (s)(r)
ConcatenationRegex != ConcatenationRegex + ClosureRegex / (
    ast.ConcatenationNode,
    (0, 1),
) | ClosureRegex

# (s)∗  (s)+  (s)? ()
ClosureRegex != ContentRegex + star / (
    ast.ClousureStarNode,
    (0,),
) | ContentRegex + plus / (ast.ClousurePlusNode, (0,)) | ContentRegex + ask / (
    ast.ClousureMayNode,
    (0,),
) | ContentRegex
...
SetRegex != SetRegex + ElemSetRegex / (ast.ElemSetComNode, (0, 1)) | ElemSetRegex / (
    ast.ElemSetComNode,
    (0, 0),
)

ElemSetRegex != char + minus + char / (ast.RangeNode, (0, 2)) | AnyEnfRegex | char

AnyEnfRegex != esc + char / (ast.AnyEscaperNode, (0, 1)) | esc + Reserved / (
    ast.AnyEscaperNode,
    (0, 1),
