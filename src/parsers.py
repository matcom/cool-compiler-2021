    ...
    while True:
        ...
        if action == table.Action.SHIFT:
            ...
            return (
                None,
                [
                    f'({line_error}, {col_error}) - SyntacticError: ERROR at or near "{token.lexeme}"'
                ],
            )
...
def parser_lr1(grammar):
    ...
    automaton = Automaton.to_deterministic(
        start_value,
        grammar.goto_lr1,
        grammar.clousure_lr1,
        grammar.state_constr,
        grammar.vocabulary,
    )
            ...
            if item.is_reduce:
                ...
                if item.production.left == grammar.start_symbol:
                    ...
                    for lookahead in item.lookaheads:
                        ...
                        value = (
                            table.Action.REDUCE,
                            (item.production.left.name, right_s_count, attr),
                        )
                        table.action(key, value)
            else:
    ...
    r = 0
    for c in keys:
        if c[0] == table.Action.REDUCE:
            r += 1
        if c[0] == table.Action.SHIFT:
            s += 1
