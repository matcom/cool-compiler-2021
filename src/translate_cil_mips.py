...
class CILToMIPSVisitor:
    ...
    def register_runtime_errors(self):
        self.runtime_errors[
            "dispatch_void"
        ] = "Runtime Error: A dispatch (static or dynamic) on void"
        self.runtime_errors["case_void"] = "Runtime Error: A case on void"
        self.runtime_errors[
            "case_no_match"
        ] = "Runtime Error: Execution of a case statement without a matching branch"
    ...
    def visit(self, node):
        ...
        for idx, var in enumerate(
            self.current_function.localvars + self.current_function.params
        ):
        ...
        self.text += (
            f"addi $sp, $sp, {-4 * len(node.localvars)}\n"  # save space for locals
        )
    ...
    def visit(self, node):
        ...
        if node.right_expr:
            if isinstance(node.right_expr, int):
                ...
                right_offset = self.var_offset[self.current_function.name][
                    node.right_expr
                ]
    ...
    def visit(self, node):
        ...
        if node.value:
            value_offset = self.var_offset[self.current_function.name][
                node.value
