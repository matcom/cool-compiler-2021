...
_lr_productions = [
  ...
  ('class_list -> def_class','class_list',1,'p_class_list_single','parser.py',33),
  ('class_list -> def_class class_list','class_list',2,'p_class_list_multi','parser.py',38),
  ('def_class -> CLASS TYPEID OCUR feature_list CCUR SEMICOLON','def_class',6,'p_def_class','parser.py',43),
  ('def_class -> CLASS TYPEID INHERITS TYPEID OCUR feature_list CCUR SEMICOLON','def_class',8,'p_def_class_parent','parser.py',48),
  ('feature_list -> <empty>','feature_list',0,'p_feature_list_empty','parser.py',53),
  ('feature_list -> def_attr feature_list','feature_list',2,'p_feature_list_attr','parser.py',59),
  ('feature_list -> def_func feature_list','feature_list',2,'p_feature_list_fun','parser.py',64),
  ('def_attr -> OBJECTID COLON TYPEID SEMICOLON','def_attr',4,'p_attr','parser.py',69),
  ('def_attr -> OBJECTID COLON TYPEID ASSIGN exp SEMICOLON','def_attr',6,'p_attr_exp','parser.py',74),
  ('def_func -> OBJECTID OPAR CPAR COLON TYPEID OCUR exp CCUR SEMICOLON','def_func',9,'p_func','parser.py',79),
  ('def_func -> OBJECTID OPAR param_list CPAR COLON TYPEID OCUR exp CCUR SEMICOLON','def_func',10,'p_func_param','parser.py',84),
  ('param_list -> param','param_list',1,'p_param_list_single','parser.py',89),
  ('param_list -> param COMMA param_list','param_list',3,'p_param_list_multi','parser.py',94),
  ('param -> OBJECTID COLON TYPEID','param',3,'p_param','parser.py',99),
  ('exp -> OBJECTID ASSIGN exp','exp',3,'p_exp_assign','parser.py',104),
  ('exp -> LET ident_list IN exp','exp',4,'p_exp_let','parser.py',114),
  ('ident_list -> iden','ident_list',1,'p_ident_list_single','parser.py',120),
  ('ident_list -> iden COMMA ident_list','ident_list',3,'p_ident_list_multi','parser.py',125),
  ('iden -> OBJECTID COLON TYPEID','iden',3,'p_iden','parser.py',130),
  ('iden -> OBJECTID COLON TYPEID ASSIGN exp','iden',5,'p_iden_init','parser.py',135),
  ('case_list -> branch','case_list',1,'p_case_list_single','parser.py',142),
  ('case_list -> branch case_list','case_list',2,'p_case_list_multi','parser.py',147),
  ('branch -> OBJECTID COLON TYPEID CASSIGN exp SEMICOLON','branch',6,'p_branch','parser.py',152),
  ('exp -> NOT exp','exp',2,'p_exp_not','parser.py',157),
  ('exp -> comp','exp',1,'p_exp_comp','parser.py',163),
  ('comp -> arith','comp',1,'p_comp_arith','parser.py',169),
  ('comp -> arith LOWER arith','comp',3,'p_comp_lower','parser.py',174),
  ('comp -> arith LEQ arith','comp',3,'p_comp_leq','parser.py',179),
  ('comp -> arith EQUAL arith','comp',3,'p_comp_equal','parser.py',184),
  ('comp -> arith EQUAL NOT exp','comp',4,'p_comp_equal_not','parser.py',189),
  ('arith -> term','arith',1,'p_arith_term','parser.py',194),
  ('arith -> arith PLUS term','arith',3,'p_arith_plus','parser.py',202),
  ('arith -> arith MINUS term','arith',3,'p_arith_minus','parser.py',207),
  ('term -> factor','term',1,'p_term_fac','parser.py',212),
  ('term -> term STAR factor','term',3,'p_term_star','parser.py',217),
  ('term -> term DIV factor','term',3,'p_term_div','parser.py',222),
  ('factor -> atom','factor',1,'p_factor_atom','parser.py',227),
  ('factor -> TILDE factor','factor',2,'p_factor_neg','parser.py',232),
  ('factor -> CASE exp OF case_list ESAC','factor',5,'p_factor_case','parser.py',237),
  ('factor -> WHILE exp LOOP exp POOL','factor',5,'p_factor_while','parser.py',242),
  ('factor -> OCUR exp_list CCUR','factor',3,'p_factor_block','parser.py',247),
  ('exp_list -> exp SEMICOLON','exp_list',2,'p_exp_list_single','parser.py',252),
  ('exp_list -> exp SEMICOLON exp_list','exp_list',3,'p_exp_list_multi','parser.py',257),
  ('factor -> IF exp THEN exp ELSE exp FI','factor',7,'p_factor_cond','parser.py',264),
  ('factor -> ISVOID factor','factor',2,'p_factor_void','parser.py',269),
  ('atom -> INT_CONST','atom',1,'p_atom_num','parser.py',275),
  ('atom -> STRING_CONST','atom',1,'p_atom_string','parser.py',280),
  ('atom -> TRUE','atom',1,'p_atom_bool','parser.py',285),
  ('atom -> FALSE','atom',1,'p_atom_bool','parser.py',286),
  ('atom -> OBJECTID','atom',1,'p_atom_var','parser.py',292),
  ('atom -> NEW TYPEID','atom',2,'p_atom_new','parser.py',297),
  ('atom -> func_call','atom',1,'p_atom_func_call','parser.py',302),
  ('atom -> OPAR exp CPAR','atom',3,'p_atom_exp','parser.py',307),
  ('func_call -> OBJECTID OPAR arg_list CPAR','func_call',4,'p_func_call_self','parser.py',312),
  ('func_call -> atom DOT OBJECTID OPAR arg_list CPAR','func_call',6,'p_func_call','parser.py',317),
  ('func_call -> atom AT TYPEID DOT OBJECTID OPAR arg_list CPAR','func_call',8,'p_func_call_at','parser.py',322),
  ('arg_list -> <empty>','arg_list',0,'p_arg_list_empty','parser.py',327),
  ('arg_list -> arg_list_not_empty','arg_list',1,'p_arg_list_not_empty','parser.py',333),
  ('arg_list_not_empty -> exp','arg_list_not_empty',1,'p_arg_list_not_empty_single','parser.py',338),
  ('arg_list_not_empty -> exp COMMA arg_list_not_empty','arg_list_not_empty',3,'p_arg_list_not_empty_multi','parser.py',343),
]
