...
class Register(str, Enum):
    ...
    # used for passing arguments to procedures,temporary (not preserved across call)
    a0 = "$a0" 
    a1 = "$a1"
    a2 = "$a2"
    a3 = "$a3"

    # used for local storage; calling procedure saves these,saved temporary (preserved across call)
    t0 = "$t0"
    t1 = "$t1"
    t2 = "$t2"
    t3 = "$t3"
    ...
    def inner(func):
        ...
        def wrapper(self: "Mips", *args, **kwargs):
            ...
            if instruction is not None:
                self.write_inst(instruction, tabs)
...
# First,  load from to address `0($sp)` and then write `addi $sp ,  $sp ,  8` to restore the stack pointer
