...
_lr_productions = [
  ...
  ('program -> class_list','program',1,'p_program','parser.py',33),
  ('class_list -> class_def SEMI class_list','class_list',3,'p_class_list','parser.py',40),
  ('class_list -> class_def SEMI','class_list',2,'p_class_list','parser.py',41),
  ('class_def -> CLASS TYPE INHERITS TYPE OCUR feature_list CCUR','class_def',7,'p_class_def','parser.py',51),
  ('class_def -> CLASS TYPE OCUR feature_list CCUR','class_def',5,'p_class_def','parser.py',52),
  ('feature_list -> attr_def SEMI feature_list','feature_list',3,'p_feature_list','parser.py',64),
  ('feature_list -> func_def SEMI feature_list','feature_list',3,'p_feature_list','parser.py',65),
  ('feature_list -> empty','feature_list',1,'p_feature_list','parser.py',66),
  ('attr_def -> ID COLON TYPE ASSIGN expr','attr_def',5,'p_attr_def','parser.py',76),
  ('attr_def -> ID COLON TYPE','attr_def',3,'p_attr_def','parser.py',77),
  ('func_def -> ID OPAR param_list CPAR COLON TYPE OCUR expr CCUR','func_def',9,'p_func_def','parser.py',89),
  ('func_def -> ID OPAR CPAR COLON TYPE OCUR expr CCUR','func_def',8,'p_func_def','parser.py',90),
  ('param_list -> ID COLON TYPE COMMA param_list','param_list',5,'p_param_list','parser.py',102),
  ('param_list -> ID COLON TYPE','param_list',3,'p_param_list','parser.py',103),
  ...
  ('expr -> IF expr THEN expr ELSE expr FI','expr',7,'p_expr_if','parser.py',170),
  ('expr -> WHILE expr LOOP expr POOL','expr',5,'p_expr_while','parser.py',179),
  ('block -> expr SEMI block','block',3,'p_block','parser.py',188),
  ('block -> expr SEMI','block',2,'p_block','parser.py',189),
  ('expr -> OCUR block CCUR','expr',3,'p_expr_block','parser.py',199),
  ('decl_list -> ID COLON TYPE ASSIGN expr COMMA decl_list','decl_list',7,'p_decl_list','parser.py',208),
  ('decl_list -> ID COLON TYPE COMMA decl_list','decl_list',5,'p_decl_list','parser.py',209),
  ('decl_list -> ID COLON TYPE ASSIGN expr','decl_list',5,'p_decl_list','parser.py',210),
  ('decl_list -> ID COLON TYPE','decl_list',3,'p_decl_list','parser.py',211),
  ('expr -> LET decl_list IN expr','expr',4,'p_expr_let','parser.py',226),
  ('case_list -> ID COLON TYPE ARROW expr SEMI case_list','case_list',7,'p_case_list','parser.py',235),
  ('case_list -> ID COLON TYPE ARROW expr SEMI','case_list',6,'p_case_list','parser.py',236),
  ('expr -> CASE expr OF case_list ESAC','expr',5,'p_expr_case','parser.py',246),
  ('expr -> NEW TYPE','expr',2,'p_expr_new','parser.py',256),
  ('expr -> ISVOID expr','expr',2,'p_expr_isvoid','parser.py',265),
  ('expr -> expr PLUS expr','expr',3,'p_expr_binary_op','parser.py',274),
  ('expr -> expr MINUS expr','expr',3,'p_expr_binary_op','parser.py',275),
  ('expr -> expr STAR expr','expr',3,'p_expr_binary_op','parser.py',276),
  ('expr -> expr DIV expr','expr',3,'p_expr_binary_op','parser.py',277),
  ('expr -> expr LESS expr','expr',3,'p_expr_binary_op','parser.py',278),
  ('expr -> expr LEQ expr','expr',3,'p_expr_binary_op','parser.py',279),
  ('expr -> expr EQ expr','expr',3,'p_expr_binary_op','parser.py',280),
  ('expr -> COMP expr','expr',2,'p_expr_comp','parser.py',300),
  ('expr -> NOT expr','expr',2,'p_expr_not','parser.py',307),
  ('expr -> OPAR expr CPAR','expr',3,'p_expr_pars','parser.py',316),
  ('expr -> ID','expr',1,'p_expr_id','parser.py',323),
  ('expr -> INT','expr',1,'p_expr_int','parser.py',332),
  ('expr -> STRING','expr',1,'p_expr_string','parser.py',341),
  ('expr -> BOOL','expr',1,'p_expr_bool','parser.py',350),
