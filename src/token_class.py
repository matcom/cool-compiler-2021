    ...
    while pos < l:
        ...
        if not matched:
            if n_pos == l and len(lexeme):  # EOF CASE
                errors.append(
                    f"({n_line}, {n_column}) - LexicographicError: ERROR EOF in {id} "
                )
            elif not len(lexeme):  # No matched character
                errors.append(
                    f'({line}, {column}) - LexicographicError: ERROR in "{char}" '
                )
            else:  # no scaped, null...
                errors.append(
                    f'({n_line}, {n_column}) - LexicographicError: ERROR in "{lexeme}" '
                )
        else:
            if not consider[0]:
...
def no_admited_calculator(lexeme: str, specials, line, column):
    if not len(lexeme) or not len(specials) > 1 or not len(specials[1]):
        return []
    n_errors = []
    for x in specials[1]:
        ...
        if count:
            ...
            while len(sub) and count:
                ...
                before = sub[:ind]
                sub = sub[ind + 1 :]
                _, temp_line, temp_column = lines_column_calculator(
                    temp_line, temp_column, 0, before
                )
                n_errors.append(
                    f"({temp_line}, {temp_column}) - LexicographicError: ERROR CONTAIN {x}"
                )
                temp_column += 1
    return n_errors
