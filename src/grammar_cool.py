...
def attr_decoder(attr, symbols_to_reduce, ast_class):
    ...
    if attr:
        if len(attr) == 2:
            ...
            args = list(
                map(
                    lambda i: symbols_to_reduce[i].tag
                    if symbols_to_reduce[i] is not None
                    else None,
                    attr_pos,
                )
            )
            ...
            if (
                ...
                and len(args[0])
                and hasattr(args[0][0], "line_no")
                and args[0][0].line_no != -1
            ):
        ...
        if len(attr) == 1:
            if isinstance(attr[0], int):
                ...
                attr_class = attr[0]
                arg = (
                    symbols_to_reduce[0].tag
                    if len(symbols_to_reduce) and symbols_to_reduce[0] is not None
                    else None
                )
...
#### TERMINALS ####
# Ignore Tokens \t\r\f
line_break, tab, nxt_page, space = cool_grammar.terminal(
    (to_ast.TOKEN_TYPE.LINEBREAK, "\n|\r", (True,)),
    (to_ast.TOKEN_TYPE.TAB, "\t", (True,)),
    (to_ast.TOKEN_TYPE.NXTPAGE, "\f", (True,)),
    (to_ast.TOKEN_TYPE.SPACE, " ", (True,)),
)
# Operators / * + -
div, mult, plus, minus = cool_grammar.terminal(
    (to_ast.TOKEN_TYPE.DIV, "\/"),
    (to_ast.TOKEN_TYPE.MULT, "\*"),
    (to_ast.TOKEN_TYPE.PLUS, "\+"),
    (to_ast.TOKEN_TYPE.MINUS, "\-"),
)
# Comparators & Assigns = < <= <- , => ~
eq, lt, lteq, assign, action, int_comp = cool_grammar.terminal(
    (to_ast.TOKEN_TYPE.EQ, "="),
...
# gropers ( ) [ ]
lparen, rparen, lbrace, rbrace = cool_grammar.terminal(
    (to_ast.TOKEN_TYPE.LPAREN, "\("),
...
# Separators
dot, comma, colon, semicolon = cool_grammar.terminal(
    (to_ast.TOKEN_TYPE.DOT, "\."),
...
class_, inherits, new_, isvoid = cool_grammar.terminal(
    (to_ast.TOKEN_TYPE.CLASS, "[Cc][Ll][Aa][Ss][Ss]"),
    (to_ast.TOKEN_TYPE.INHERITS, "[Ii][Nn][Hh][Ee][Rr][Ii][Tt][Ss]"),
    (to_ast.TOKEN_TYPE.NEW, "[Nn][Ee][Ww]"),
    (to_ast.TOKEN_TYPE.ISVOID, "[Ii][Ss][Vv][Oo][Ii][Dd]"),
)

in_, case, of, esac = cool_grammar.terminal(
    (to_ast.TOKEN_TYPE.IN, "[Ii][Nn]"),
    (to_ast.TOKEN_TYPE.CASE, "[Cc][Aa][Ss][Ee]"),
    (to_ast.TOKEN_TYPE.OF, "[Oo][Ff]"),
    (to_ast.TOKEN_TYPE.ESAC, "[Ee][Ss][Aa][Cc]"),
)

if_, then, else_, fi = cool_grammar.terminal(
    (to_ast.TOKEN_TYPE.IF, "[Ii][Ff]"),
    (to_ast.TOKEN_TYPE.THEN, "[Tt][Hh][Ee][Nn]"),
    (to_ast.TOKEN_TYPE.ELSE, "[Ee][Ll][Ss][Ee]"),
    (to_ast.TOKEN_TYPE.FI, "[Ff][Ii]"),
)

while_, loop_, pool = cool_grammar.terminal(
    (to_ast.TOKEN_TYPE.WHILE, "[Ww][Hh][Ii][Ll][Ee]"),
    (to_ast.TOKEN_TYPE.LOOP, "[Ll][Oo][Oo][Pp]"),
    (to_ast.TOKEN_TYPE.POOL, "[Pp][Oo][Oo][Ll]"),
)

let, not_ = cool_grammar.terminal(
    (to_ast.TOKEN_TYPE.LET, "[Ll][Ee][Tt]"), (to_ast.TOKEN_TYPE.NOT, "[Nn][Oo][Tt]")
)
...
obj_id = cool_grammar.terminal((to_ast.TOKEN_TYPE.OBJECT_ID, "[a-z][a-zA-Z0-9_]*"))
...
multiline_comment = cool_grammar.terminal(
    (
        to_ast.TOKEN_TYPE.MULTILINECOMMENT,
        r"(\(\*)(([^\*]*)(\*[^\)\*])?)*(\*+\))",
        (True,),
    )
)
...
(
    ...
    Attrs_def,
    Attr_def,
) = cool_grammar.nonTerminals(
    *"Program,Class_list,Class_def,Feature_list,Meth_def,"
...
(
    ...
    *"Type,Expr,Param_list,Param,Other_param,Comparer,Open_expr_lvl1".split(",")
)
...
(
    ...
    *"Arith,Open_expr_lvl2,Term,Open_expr_lvl3,Open_expr,Let_var_list".split(",")
)

Let_var, Let_var_assign, Branch, Factor, Atom, Bool = cool_grammar.nonTerminals(
    *"Let_var,Let_var_assign,Branch,Factor,Atom,Bool".split(",")
)
...
(
    ...
    *"Open_Expr,Expr_list,Branch_list,Func_call,Arg_list,Other_arg".split(",")
)
...
Class_list != Class_def / (to_ast.ListNode,) | Class_def + Class_list / (
    to_ast.ListNode,
    (0, 1),
)

Class_def != class_ + type_id + lbrace + Feature_list + rbrace + semicolon / (
    to_ast.ClassDeclarationWrapper,
    (1, 3),
) | class_ + type_id + inherits + type_id + lbrace + Feature_list + rbrace + semicolon / (
...
Feature_list != Attrs_def + semicolon + Feature_list / (
    ...
    to_ast.ListNode,
    (0, 2),
) | cool_grammar.epsilon / (
    to_ast.ListNode,
)

Attrs_def != Attr_def | Attr_def + comma + Attrs_def / (to_ast.ListNode, (0, 2))

Attr_def != obj_id + colon + Type / (
    ast.AttrDeclarationNode,
    (0, 2),
) | obj_id + colon + Type + assign + Expr / (ast.AttrDeclarationNode, (0, 2, 4))

Meth_def != obj_id + lparen + Param_list + rparen + colon + Type + lbrace + Expr + rbrace / (
    ast.FuncDeclarationNode,
    (0, 2, 5, 7),
)

Param_list != Param + Other_param / (to_ast.ListNode, (0, 1)) | cool_grammar.epsilon / (
    to_ast.ListNode,
)
...
Expr != Comparer + lt + Open_expr_lvl1 / (
    ...
    ast.EqualNode,
    (0, 2),
) | Open_expr_lvl1 | Comparer

Open_expr_lvl1 != Arith + plus + Open_expr_lvl2 / (
    ast.PlusNode,
    (0, 2),
) | Arith + minus + Open_expr_lvl2 / (ast.MinusNode, (0, 2)) | Open_expr_lvl2

Open_expr_lvl2 != Term + mult + Open_expr_lvl3 / (
    ast.StarNode,
    (0, 2),
) | Term + div + Open_expr_lvl3 / (ast.DivNode, (0, 2)) | Open_expr_lvl3

Open_expr_lvl3 != isvoid + Open_expr_lvl3 / (
    ast.IsVoidNode,
    (1,),
) | int_comp + Open_expr_lvl3 / (ast.IntCompNode, (1,)) | Open_expr

Open_expr != let + Let_var_list + in_ + Expr / (
    ast.LetNode,
    (1, 3),
) | obj_id + assign + Expr / (ast.AssignNode, (0, 2)) | not_ + Expr / (
    ast.NotNode,
    (1,),
)

Comparer != Comparer + lt + Arith / (ast.LessNode, (0, 2)) | Comparer + lteq + Arith / (
    ast.LessEqualNode,
    (0, 2),
) | Comparer + eq + Arith / (ast.EqualNode, (0, 2)) | Arith

Arith != Arith + plus + Term / (ast.PlusNode, (0, 2)) | Arith + minus + Term / (
    ast.MinusNode,
    (0, 2),
) | Term

Term != Term + mult + Factor / (ast.StarNode, (0, 2)) | Term + div + Factor / (
    ast.DivNode,
    (0, 2),
) | Factor

Factor != isvoid + Factor / (ast.IsVoidNode, (1,)) | int_comp + Factor / (
    ast.IntCompNode,
    (1,),
) | Resoluted
# |Atom

Resoluted != Resoluted + dot + obj_id + lparen + Arg_list + rparen / (
    ast.CallNode,
    (0, 2, 4),
) | Resoluted + at + type_id + dot + obj_id + lparen + Arg_list + rparen / (
    ast.CallNode,
    (0, 4, 6, 2),
) | Atom

Atom != integer / (ast.ConstantNumNode,) | obj_id / (ast.VariableNode,) | string / (
    to_ast.StringWrapper,
) | Bool / (ast.BoolNode,) | lparen + Expr + rparen / (1,) | new_ + type_id / (
    ast.InstantiateNode,
    (1,),
) | if_ + Expr + then + Expr + else_ + Expr + fi / (
    ast.ConditionalNode,
    (1, 3, 5),
) | while_ + Expr + loop_ + Expr + pool / (
    ast.LoopNode,
    (1, 3),
...
Let_var_list != Let_var / (to_ast.ListNode,) | Let_var + comma + Let_var_list / (
    to_ast.ListNode,
    (0, 2),
) | Let_var_assign / (to_ast.ListNode,) | Let_var_assign + comma + Let_var_list / (
    to_ast.ListNode,
    (0, 2),
)
...
Let_var_assign != obj_id + colon + Type + assign + Expr / (
    to_ast.TupleWrapper,
    (0, 2, 4),
)

Branch_list != Branch + Branch_list / (to_ast.ListNode, (0, 1)) | Branch / (
    to_ast.ListNode,
)

Branch != obj_id + colon + Type + action + Expr + semicolon / (
    ast.BranchNode,
    (0, 2, 4),
)
...
Arg_list != Expr + Other_arg / (to_ast.ListNode, (0, 1)) | cool_grammar.epsilon / (
    to_ast.ListNode,
)
