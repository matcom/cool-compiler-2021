...
class SoftInferencer:
    ...
    def visit(self, node: ProgramNode) -> inf_ast.ProgramNode:
        return inf_ast.ProgramNode.shallow_infer(node, Scope(), self)

    @visitor.when(ClassNode)
    def visit(self, node: ClassNode, scope: Scope) -> ClassNode:
        return inf_ast.ClassDeclarationNode.shallow_infer(node, scope, self)

    @visitor.when(AttrDeclNode)
    def visit(self, node: AttrDeclNode, scope: Scope):
        return inf_ast.AttrDeclarationNode.shallow_infer(node, scope, self)

    @visitor.when(MethodDeclNode)
    def visit(self, node: MethodDeclNode, scope: Scope):
        return inf_ast.MethodDeclarationNode.shallow_infer(node, scope, self)

    @visitor.when(BlockNode)
    def visit(self, node: BlockNode, scope):
        return inf_ast.BlocksNode.shallow_infer(node, scope, self)

    @visitor.when(IfThenElseNode)
    def visit(self, node: IfThenElseNode, scope):
        return inf_ast.ConditionalNode.shallow_infer(node, scope, self)

    @visitor.when(CaseNode)
    def visit(self, node: CaseNode, scope: Scope):
        return inf_ast.CaseNode.shallow_infer(node, scope, self)

    @visitor.when(CaseBranchNode)
    def visit(self, node: CaseBranchNode, scope: Scope):
        return inf_ast.CaseOptionNode.shallow_infer(node, scope, self)

    @visitor.when(WhileNode)
    def visit(self, node: WhileNode, scope):
        return inf_ast.LoopNode.shallow_infer(node, scope, self)

    @visitor.when(LetInNode)
    def visit(self, node: LetInNode, scope: Scope):
        return inf_ast.LetNode.shallow_infer(node, scope, self)

    @visitor.when(LetDeclNode)
    def visit(self, node: LetDeclNode, scope: Scope):
        return inf_ast.VarDeclarationNode.shallow_infer(node, scope, self)

    @visitor.when(AssignNode)
    def visit(self, node: AssignNode, scope: Scope):
        return inf_ast.AssignNode.shallow_infer(node, scope, self)

    @visitor.when(StaticDispatchNode)
    def visit(self, node: StaticDispatchNode, scope):
        return inf_ast.MethodCallNode.shallow_infer_static(node, scope, self)

    @visitor.when(DispatchNode)
    def visit(self, node: DispatchNode, scope):
        return inf_ast.MethodCallNode.shallow_infer_dynamic(node, scope, self)

    @visitor.when(ArithmeticNode)
    def visit(self, node: ArithmeticNode, scope):
        return inf_ast.ArithmeticNode.shallow_infer(node, scope, self)

    @visitor.when(LeNode)
    def visit(self, node: LeNode, scope: Scope):
        return inf_ast.LessNode.shallow_infer(node, scope, self)

    @visitor.when(LeqNode)
    def visit(self, node, scope: Scope):
        return inf_ast.LessOrEqualNode.shallow_infer(node, scope, self)

    @visitor.when(EqNode)
    def visit(self, node: EqNode, scope: Scope):
        return inf_ast.EqualsNode.shallow_infer(node, scope, self)

    @visitor.when(VarNode)
    def visit(self, node: VarNode, scope: Scope):
        return inf_ast.VariableNode.shallow_infer(node, scope, self)

    @visitor.when(NotNode)
    def visit(self, node: NotNode, scope):
        return inf_ast.NotNode.shallow_infer(node, scope, self)

    @visitor.when(TildeNode)
    def visit(self, node: TildeNode, scope):
        return inf_ast.ComplementNode.shallow_infer(node, scope, self)

    @visitor.when(IsVoidNode)
    def visit(self, node: IsVoidNode, scope):
        return inf_ast.IsVoidNode.shallow_infer(node, scope, self)

    @visitor.when(NewNode)
    def visit(self, node: NewNode, scope):
        return inf_ast.InstantiateNode.shallow_infer(node, scope, self)

    @visitor.when(IntNode)
    def visit(self, node: IntNode, scope):
        return inf_ast.IntNode.shallow_infer(node, scope, self)

    @visitor.when(StringNode)
    def visit(self, node: StringNode, scope):
        return inf_ast.StringNode.shallow_infer(node, scope, self)

    @visitor.when(BoolNode)
    def visit(self, node: BoolNode, scope):
        return inf_ast.BooleanNode.shallow_infer(node, scope, self)
