from app.semantics.inference.inferencer_ast import (
    ArithmeticNode,
    AssignNode,
    AttrDeclarationNode,
    BlocksNode,
...
from app.semantics.tools.errors import AttributeError, InternalError
...
class HardInferencer:
    ...
    def visit(self, node: MethodCallNode, scope):
        ...
        if(real_caller_type_name == 'SELF_TYPE'):
            real_caller_type_name = self.current_type.name
        try:
            el_metodo_del_conte = self.context.get_type(real_caller_type_name).get_method(
                node.id)
            # print(
            # f'EEEEELLL METODO {el_metodo_del_conte.name}({real_caller_type_name})', el_metodo_del_conte)
            # print("El metodo del context tipo de retorno:",
            #       el_metodo_del_conte.return_type)
            # print(f'EEEEEELLLL CONTTEEEXXTTT: {self.context}')
            real_return_type = el_metodo_del_conte.return_type

            if(real_return_type.name == 'SELF_TYPE'):
                if(caller_type.name == 'SELF_TYPE'):
                    real_return_type = self.current_type
                else:
