...
class Type:
    ...
    def get_attribute(self, name: str, first=None):
        # print(f'Los atributos en nodo {name} son {self.attributes}')
        if not first:
            first = self.name
        ...
        try:
            # print(
            #     f'Los atributos de {self.name} buscando {name} son: {self.attributes}')
    ...
    def conforms_to(self, other, first=None):
        if isinstance(other, TypeBag):
            other = [*other.type_set][0]
        elif isinstance(first, TypeBag):
            first = [*first.type_set][0]

        # print(f'Conforming {self.name}:{self.parent} to: {other.name}')
        if not first:
            first = self.name
        elif self.name == first:
            return False

        # print("las tallas", other.bypass(), self.parent)
        return (
            other.bypass()
            or self == other
    ...
    def bypass(self):
        # print("Name en el bypass:", self.name)
        if self.name == "Object":
            return True
        return False
...
class TypeBag:
    def __init__(self, type_set, heads=[]) -> None:
        self.type_set: set = (
            type_set if isinstance(
                type_set, set) else from_dict_to_set(type_set)
        )
    ...
    def bypass(self):
        return [*self.type_set][0].bypass()

    def conforms_to(self, node_id):
        if(isinstance(node_id, TypeBag)):
            type1 = [*self.type_set][0]
            type2 = [*node_id.type_set][0]
            # print("Los tipos", type1, type2)
            return type1.conforms_to(type2)
        return [*self.type_set][0].conforms_to(node_id)

    # def conforms_to(self, node_id):
    #     return [*self.type_set][0].conforms_to(node_id)

    def get_method(self, node_id):
        return [*self.type_set][0].get_method(node_id)

    def get_attribute(self, node_id):
        return [*self.type_set][0].get_attribute(node_id)

    def all_attributes(self):
        return [*self.type_set][0].all_attributes()

    def all_methods(self):
        return [*self.type_set][0].all_methods()
    ...
    def __str__(self):
        return f"type: {self.name} type_set: {self.type_set}"
