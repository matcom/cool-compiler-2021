...
import app.utils.meta as meta
import app.utils.visitor as visitor
import app.ast.cool.type_collected as type_collected
import app.ast.cool.type_built as type_built
import app.semantic as semantic
from app.semantic import ErrorType
...
class TypeBuilderVisitor:
    ...
    def visit(self, node: type_collected.CoolClassNode):
        ...
        if node.parent is not None:
            try:
                ...
                self.errors.append(errors.TypeError(
                    node.lineno, node.columnno, e.text))

        try:
            self.current_type.set_parent(parent_type)
        except semantic.SemanticError as e:
            self.errors.append(errors.SemanticError(
                node.lineno, node.columnno, e.text))
    ...
    def visit(self, node: type_collected.CoolMethodDeclNode):
        ...
        try:
            ...
            self.errors.append(errors.TypeError(
                node.lineno, node.columnno, error.text))
