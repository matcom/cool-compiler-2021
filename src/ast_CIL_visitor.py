...
class CILVisitor:
    ...
    def visit(self, node: ProgramNode, func: Node):
        NotImplemented

    @visitor.when(ClassDeclarationNode)
    def visitor(self, node: ClassDeclarationNode, func: Node):
        NotImplemented

    @visitor.when(FuncDeclarationNode)
    def visit(self, node: FuncDeclarationNode, func: CILProgram):
        NotImplemented
    ...
    def visit(self, node: AttrDeclarationNode, func: CILFuncDeclaration):
        ...
        func.add_instruction(
            [CILALLOCATEInst(name, node.type), CILSimpleAssignInstruction(name, val)], 0
        )
        return name
    ...
    def visit(self, node: AtomicNode, func: CILFuncDeclaration):
        ...
        if isinstance(node, InstantiateNode):
        ...
        func.add_instruction(
            [
                CILALLOCATEInst(current_local, type),
                CILSETATTRInst(current_local, 0, value),
            ],
            0,
        )
    ...
    def visit(self, node: BinaryNode, func: CILFuncDeclaration):
        if isinstance(node, ArithBinaryNode):
            return self.visit_binary_arith(node, func)
        if isinstance(node, BooleanBinaryNode):
            return self.visit_binary_boolean(node, func)
    ...
    def visit(self, node: IsVoidNode, func: CILFuncDeclaration):
        ...
        func.add_instruction(
            [
                CILALLOCATEInst(current_local, "BOOL"),
                CILSETATTRInst(current_local, 0, 0),
            ],
            0,
        )
    ...
    def visit(self, node: AssignNode, func: CILFuncDeclaration):
        NotImplemented
    ...
    def visit(self, node: CallNode, func: CILFuncDeclaration):
        NotImplemented

    @visitor.when(CaseNode)
    def visit(self, node: CaseNode, func: CILFuncDeclaration):
        NotImplemented
    ...
    def visit(self, node: LoopNode, func: CILFuncDeclaration):
        ...
        func.add_instruction(
            [CILIFGOTOInst(cond, l2), CILGOTOInst(l3), CILLABELInst(l2)]
        )
    ...
    def visit(self, node: ConditionalNode, func: CILFuncDeclaration):
        ...
        func.add_instruction(
            [CILIFGOTOInst(out_if_expr, then_label), CILGOTOInst(out_else_expr)]
        )  # WHEN COND IS TRUE
        return current_local
