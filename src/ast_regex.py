...
class ElemSetComNode(BinaryNode):
    ...
    def shift(self):
        left = (
            any_escaper(self.left.left, self.left.right)
            if isinstance(self.left, AnyEscaperNode)
            else None
        )
        left = {self.left.lexeme} if isinstance(self.left, Token) else left

        right = (
            any_escaper(self.right.left, self.right.right)
            if isinstance(self.right, AnyEscaperNode)
            else None
        )
