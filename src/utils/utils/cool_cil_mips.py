...
class BaseCoolToCilVisitor:
    ...
    def add_function_assign(self): 
        ...
        constant_null = self.define_internal_local()
        is_null = self.define_internal_local()
        type_of_source = self.define_internal_local()
        type_int = self.define_internal_local()
        type_bool = self.define_internal_local()
...
class CoolToCilTranslator(BaseCoolToCilVisitor):
    ...
    def visit(self, node: cool.CaseNode, scope: Scope):
        ...
        constant_zero_int = self.define_internal_local()
        constant_one_int = self.define_internal_local()
        constant_len_types_int = self.define_internal_local()
        constant_null_ptr = self.define_internal_local()
        count_of_ancestors_int = self.define_internal_local()
        step1_comparison_result_bool = self.define_internal_local()
        switch_expr_type_address = self.define_internal_local()
        ...
        self.register_instruction(cil.TypeOfNode(switch_expr_type_address, swicth_expression))
        self.register_instruction(cil.AssignNode(ancestor_type_address, switch_expr_type_address))
        ...
        step2_iter_index_int = self.define_internal_local()
        step2_comparison_result_bool = self.define_internal_local()
        ...
        self.register_instruction(cil.SetIndexNode(ancestors_array, step2_iter_index_int, ancestor_type_address))
        self.register_instruction(cil.ParentNode(ancestor_type_address, ancestor_type_address))
        ...
        step4_index_int = self.define_internal_local()
        step4_current_min_index_int = self.define_internal_local()
        step4_temp_int = self.define_internal_local()
        step4_current_min_int = self.define_internal_local()
        step4_comparison_bool = self.define_internal_local()
        ...
        self.register_instruction(cil.GetValueInIndexNode(step4_temp_int, nearest_ancestor_array, step4_index_int))
        ...
        self.register_instruction(cil.ArrayNode(bool_array, constant_len_types_int))
        for i, _ in enumerate(types):
            x = self.define_internal_local()
            self.register_instruction(cil.AllocateIntNode(x, f"{i}"))
            self.register_instruction(cil.SetValueInIndexNode(bool_array, x, constant_zero_int))
        ...
        self.register_instruction(cil.SetValueInIndexNode(bool_array, step4_current_min_index_int, constant_one_int))
 
        
        step5_comparison = self.define_internal_local()
        self.register_instruction(cil.AllocateBoolNode(step5_comparison, "0"))
        self.register_empty_instruction()
        for i_int, t in enumerate(types):
            x = self.define_internal_local()
            self.register_instruction(cil.AllocateIntNode(x, f"{i_int}"))
            self.register_instruction(cil.GetValueInIndexNode(step5_comparison, bool_array, x))
            self.register_instruction(cil.GotoIfNode(step5_comparison, f"branch_{t.name}_{node_id}"))
            self.register_empty_instruction()
    ...
    def visit(self, node: cool.NewNode, scope: Scope):
        ...
        self.register_instruction(cil.StaticCallNode(self.to_function_name("__init__", node.type), local, 1))
...
class CilToMipsTranslator(BaseCilToMipsVisitor):
    ...
    def visit(self, node: cil.FunctionNode):
        ...
        if self.current_function.localvars:
            self.register_comment("SPACE FOR LOCAL VARIABLES")
    ...
    def visit(self, node: cil.AssignIntNode):
        self.register_comment(f"{node.dest} <- {node.source} WHERE {node.source} IS AN INT")
    ...
    def visit(self, node: cil.ArgNode):
        if node.arg_index == 0:
            self.register_comment("FUNCTION ARG")
    ...
    def visit(self, node: cil.DynamicCallNode):
        self.register_comment(f"CALLING DYNAMIC FUNCTION -> {node.method_addr}")
    ...
    def visit(self, node: cil.StaticCallNode):
        self.register_comment(f"CALLING STATIC FUNCTION {node.function}")
    ...
    def visit(self, node: cil.ConcatNode):
        ...
        self.register_instruction(mips.XorNode("$t6", "$t6", "$t6"))
    ...
    def visit(self, node: cil.SubstringNode):
        ...
        self.register_instruction(mips.LoadWordNode("$t0", f"{self.offset_of(node.str_address)}($sp)"))
        self.register_instruction(mips.LoadWordNode("$t1", f"4($t0)"))
        self.register_instruction(mips.AddiNode("$t1", "$t1", "-9"))
        self.register_instruction(mips.LoadWordNode("$t2", f"{self.offset_of(node.start)}($sp)"))
        self.register_instruction(mips.LoadWordNode("$t2", "8($t2)"))
        self.register_instruction(mips.LoadWordNode("$t3", f"{self.offset_of(node.length)}($sp)"))
        ...
        self.register_instruction(mips.StoreWordNode("$t5", f"0($v0)"))
        ...
        self.register_instruction(mips.AddNode("$t0", "$t0", "$t2"))
        self.register_instruction(mips.MoveNode("$t5", "$v0"))
        self.register_instruction(mips.AddNode("$t5", "$t5", "8"))
        self.register_instruction(mips.XorNode("$t6", "$t6", "$t6"))
    ...
    def visit(self, node: cil.GetAttribNode):
        ...
        self.register_instruction(mips.LoadWordNode("$t1", f"{4 * (node.attr_index + 2)}($t0)"))
    ...
    def visit(self, node: cil.SetAttribNode):
        ...
        self.register_instruction(mips.LoadWordNode("$t0", f"{self.offset_of(node.instance)}($sp)"))
        self.register_instruction(mips.LoadWordNode("$t1", f"{self.offset_of(node.source)}($sp)"))
    ...
    def visit(self, node: cil.GetIndexNode):
        #{node.dest} <- ARRAY {node.instance}[4 * {node.index}]
    ...
    def visit(self, node: cil.SetIndexNode):
        # ARRAY {node.instance}[4 * {node.index}] <- {node.source}
    ...
    def visit(self, node: cil.TypeOfNode):
        # se guarda en la 1era word
        self.register_comment(f"{node.dest} <- TYPEOF {node.obj}")
    ...
    def visit(self, node: cil.TypeNameNode):
        ...
        self.register_instruction(mips.AddiNode("$t4", "$v0", 0))
        self.register_instruction(mips.AddiNode("$t4", "$t4", "8"))
        self.register_instruction(mips.XorNode("$t5", "$t5", "$t5"))
        ...
        self.register_instruction(mips.LoadByteNode("$t6", "0($t3)"))
        ...
        self.register_instruction(mips.StoreByteNode("$zero", "0($t4)"))
        self.register_empty_instruction()
    ...
    def visit(self, node: cil.CopyNode):
        ...
        self.register_instantiation("$t2")
        self.register_instruction(mips.MoveNode("$t3", "$v0"))
        self.register_instruction(mips.StoreWordNode("$t1", "0($v0)"))
        self.register_instruction(mips.StoreWordNode("$a0", "4($v0)"))
        self.register_empty_instruction()
    ...
    def visit(self, node: cil.PrintStringNode):
        ...
        self.register_comment(f"PRINTING STRING: {node.str_addr}")
    ...
    def visit(self, node: cil.PrintIntNode):
        self.register_comment(f"PRINTING INT: {node.int_source}")
    ...
    def visit(self, node: cil.PrintTypeNameNode):
        self.register_comment("PRINTING TYPE NAME")
        self.register_empty_instruction()
    ...
    def visit(self, node: cil.GotoNode):
        self.register_comment(f"JUMP TO {node.addr}")
    ...
    def visit(self, node: cil.PlusNode):
        self.register_comment("ADD FUNCT")
    ...
    def visit(self, node: cil.MinusNode):
        self.register_comment("SUB FUNCT")
        self.preprocess_binary_operation(node)
    ...
    def visit(self, node: cil.StarNode):
        self.register_comment("TIMES FUNCT")        
        self.preprocess_binary_operation(node)
        self.register_instruction(mips.MultNode("$t0", "$t1"))
    ...
    def visit(self, node: cil.DivNode):
        self.register_comment("DIV FUNCT")
        self.preprocess_binary_operation(node)
    ...
    def visit(self, node: cil.XorNode):
        self.register_comment("XOR FUNCT")
        self.preprocess_binary_operation(node)
    ...
    def visit(self, node: cil.LessThanNode):
        self.register_comment("LESS FUNCT")
        self.preprocess_binary_operation(node)
    ...
    def visit(self, node: cil.LessEqualNode):
        self.register_comment("LESS EQ FUNCT")
        self.preprocess_binary_operation(node)
    ...
    def visit(self, node: cil.EqualNode):
        self.register_comment("EQUAL FUNCT")
    ...
    def visit(self, node: cil.EqualStrNode):
        self.register_comment(f"{node.dest} <- EQUALSTRING ({node.left}, {node.right})")
        self.register_instruction(mips.LoadWordNode("$t0", f"{self.offset_of(node.left)}($sp)"))
        self.register_instruction(mips.LoadWordNode("$t1", f"{self.offset_of(node.right)}($sp)"))
        self.register_instruction(mips.AddiNode("$t0", "$t0", "8"))
...
class MipsFormatter1:
    ...
    def visit(self, node: mips.ProgramNode):
        ...
        instructions = [
            (f"{self.visit(inst)}" if is_function_label(inst) else f"\t{self.visit(inst)}") \
