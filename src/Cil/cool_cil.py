...
class COOLToCIL(BaseCOOLToCIL):
    ...
    def visit(self, node, scope):
        ...
        func_declarations = (f for f in node.features if isinstance(f, ast.FuncDeclarationNode))
        ...
        attr_declarations = (f for f in node.features if isinstance(f, ast.AttrDeclarationNode))
    ...
    @visitor.when(ast.IfThenElseNode)
    def visit(self, node, scope):
    ...
    @visitor.when(ast.CaseExpressionNode)
    def visit(self, node, scope):
        # node.id -> str
        # node.type -> str
        # node.expr -> ExpressionNode
    ...
    @visitor.when(ast.LetAttributeNode)
    def visit(self, node, scope):
        # node.id -> str
        # node.type -> str
        # node.expr -> ExpressionNode
    ...
    @visitor.when(ast.MemberCallNode)
    def visit(self, node, scope):
        # node.id -> str
        # node.args -> [ ExpressionNode ... ]
