...
from coolpyler.semantics.tools.scope import VariableInfo
from coolpyler.semantics.tools.errors import AttributeError
import coolpyler.semantics.inference.inferencer_ast as cool
...
class COOLToCILVisitor(BaseCOOLToCILVisitor):
    ...
    def visit(self, node: cool.CaseNode, scope):
        ...
        data_node = self.register_data(
            f'({token.row + 1 + len(node.options)},{token.column - 5}) - RuntimeError: Execution of a case statement without a matching branch\n')
        self.register_instruction(cil.ErrorNode(data_node))
    ...
    def visit(self, node: cool.MethodCallNode, scope):
        ...
        self.visit(node.static_type, scope)
        ...
        self.register_runtime_error(
            equal_result, f'({token.row},{token.column}) - RuntimeError: Dispatch on void\n')
