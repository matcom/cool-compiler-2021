...
import coolpyler.utils.visitor as visitor  # type: ignore
...
class CILDebug:
    ...
    def visit(self, node: cil.DataNode, indent=0, inline=False): # type: ignore
        self.print(indent, inline, f"{node.name} = {node.value};")

    @visitor.when(cil.FunctionNode)
    def visit(self, node: cil.FunctionNode, indent=0, inline=False): # type: ignore
    ...
    def visit(self, node: cil.ParamNode, indent=0, inline=False): # type: ignore
        self.print(indent, inline, f"PARAM {node.name};")

    @visitor.when(cil.LocalNode)
    def visit(self, node: cil.LocalNode, indent=0, inline=False): # type: ignore
        self.print(indent, inline, f"LOCAL {node.name};")

    @visitor.when(cil.PlusNode)
    def visit(self, node: cil.PlusNode, indent=0, inline=False): # type: ignore
    ...
    def visit(self, node: cil.MinusNode, indent=0, inline=False):# type: ignore
        self.print(indent, True, f"{node.dest}")
        self.print(0, True, " = ")
        self.visit(node.left, inline=True)# type: ignore
        self.print(0, True, " - ")
        self.visit(node.right, inline=inline)# type: ignore

    @visitor.when(cil.StarNode)
    def visit(self, node: cil.StarNode, indent=0, inline=False):# type: ignore
        self.print(indent, True, f"{node.dest}")
        self.print(0, True, " = ")
        self.visit(node.left, inline=True)# type: ignore
        self.print(0, True, " * ")
    ...
    def visit(self, node: cil.DivNode, indent=0, inline=False):# type: ignore
        self.print(indent, True, f"{node.dest}")
        self.print(0, True, " = ")
        self.visit(node.left, inline=True)# type: ignore
        self.print(0, True, " / ")
    ...
    def visit(self, node: cil.GetAttrNode, indent=0, inline=False):# type: ignore
        self.print(indent, inline, f"{node.dest} = GETATTR {node.instance} {node.attr};")

    @visitor.when(cil.SetAttrNode)
    def visit(self, node: cil.SetAttrNode, indent=0, inline=False):# type: ignore
        self.print(indent, inline, f"SETATTR {node.instance} {node.attr} {node.source};")

    @visitor.when(cil.GetIndexNode)
    def visit(self, node: cil.GetIndexNode, indent=0, inline=False):# type: ignore
        pass

    @visitor.when(cil.SetIndexNode)
    def visit(self, node: cil.SetIndexNode, indent=0, inline=False):# type: ignore
        pass

    @visitor.when(cil.AllocateNode)
    def visit(self, node: cil.AllocateNode, indent=0, inline=False):# type: ignore
        self.print(indent, inline, f"{node.dest} = ALLOCATE {node.type};")

    @visitor.when(cil.TypeOfNode)
    def visit(self, node: cil.TypeOfNode, indent=0, inline=False):# type: ignore
        self.print(indent, inline, f"{node.dest} = TYPEOF {node.obj};")

    @visitor.when(cil.LabelNode)
    def visit(self, node: cil.LabelNode, indent=0, inline=False):# type: ignore
        self.print(indent, inline, f"LABEL <label>;")

    @visitor.when(cil.GotoNode)
    def visit(self, node: cil.GotoNode, indent=0, inline=False):# type: ignore
        self.print(indent, inline, f"GOTO <label>;")

    @visitor.when(cil.GotoIfNode)
    def visit(self, node: cil.GotoIfNode, indent=0, inline=False):# type: ignore
        self.print(indent, inline, f"IF <cond> GOTO <label>;")

    @visitor.when(cil.StaticCallNode)
    def visit(self, node: cil.StaticCallNode, indent=0, inline=False):# type: ignore
        self.print(indent, inline, f"{node.dest} = CALL {node.function};")

    @visitor.when(cil.DynamicCallNode)
    def visit(self, node: cil.DynamicCallNode, indent=0, inline=False):# type: ignore
        self.print(indent, inline, f"{node.dest} = VCALL {node.type} {node.method};")

    @visitor.when(cil.ArgNode)
    def visit(self, node: cil.ArgNode, indent=0, inline=False):# type: ignore
        self.print(indent, inline, f"ARG {node.name};")

    @visitor.when(cil.ReturnNode)
    def visit(self, node: cil.ReturnNode, indent=0, inline=False):# type: ignore
        self.print(indent, inline, f"RETURN {node.value};")

    @visitor.when(cil.LoadNode)
    def visit(self, node: cil.LoadNode, indent=0, inline=False):# type: ignore
        self.print(indent, inline, f"{node.dest} = LOAD {node.msg};")

    @visitor.when(cil.LengthNode)
    def visit(self, node: cil.LengthNode, indent=0, inline=False):# type: ignore
        pass

    @visitor.when(cil.ConcatNode)
    def visit(self, node: cil.ConcatNode, indent=0, inline=False):# type: ignore
        self.print(indent, inline, f"CONCAT ...")

    @visitor.when(cil.PrefixNode)
    def visit(self, node: cil.PrefixNode, indent=0, inline=False):# type: ignore
        self.print(indent, inline, f"PREFIX ...")

    @visitor.when(cil.SubstringNode)
    def visit(self, node: cil.SubstringNode, indent=0, inline=False):# type: ignore
        self.print(indent, inline, f"SUBSTR ...")
        pass

    @visitor.when(cil.ToStrNode)
    def visit(self, node: cil.ToStrNode, indent=0, inline=False):# type: ignore
        self.print(indent, inline, f"STR ...")
        pass

    @visitor.when(cil.ReadNode)
    def visit(self, node: cil.ReadNode, indent=0, inline=False):# type: ignore
        self.print(indent, inline, f"READ ...")
        pass

    @visitor.when(cil.PrintNode)
    def visit(self, node: cil.PrintNode, indent=0, inline=False):# type: ignore
