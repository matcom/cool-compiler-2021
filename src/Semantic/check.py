...
class TypeCheck:
    def __init__(self, context, errors):
        ...
        self.int_type = self.context.types['Int']
        self.bool_type = self.context.types['Bool']
        self.string_type = self.context.types['String']
        self.object_type = self.context.types['Object']

        self.basic_type = [self.int_type, self.bool_type, self.string_type, self.object_type]
    ...
    def visit(self, node, scope):
        if node.id.value == 'self':
            self.errors.append(SemanticErrors(node.id.line, node.id.column, 
                f'\'self\' cannot be the name of an attribute.'))   
        if node.expr:
            ...
            if not expr_type.conforms_to(attr_type):
                self.errors.append(TypeErrors(node.expr.line,  node.expr.column, 
                    f'Inferred type {expr_type.name} of initialization of attribute {node.id.value} does not conform to declared type {attr_type.name}.'))
    ...
    def visit(self, node, scope):
        ...
        if not expression_type.conforms_to(return_type):
            self.errors.append(TypeErrors(expression_line, expression_column, 
                f'Inferred return type {expression_type.name} of method {node.id.value} does not conform to declared return type {return_type.name}.'))
    ...
    def visit(self, node, scope):
        ...
        if scope.is_defined(node.id.value, self.scope_class, self.current_type):
            ...
            if variable.name == 'self':
                self.errors.append(SemanticErrors(node.key.line, 
                node.key.column, f'Cannot assign to \'self\'.'))            
    ...
    def visit(self, node, scope):
        if node.expr:
            ...
            if node.type:
                try:
                    if expression_type.conforms_to(self.context.get_type(node.type.value)): expression_type = self.context.get_type(node.type.value)
                    else:
                        self.errors.append(TypeErrors(node.expr.line, node.expr.column, 
                        f'Expression type {expression_type.name} does not conform to declared static dispatch type {node.type.value}.'))
            ...
            if len(node.args) == len(method.param_types):
                for argument, tname, ttype in zip(node.args, method.param_names, method.param_types):
                    ...
                    if not argument_type.conforms_to(ttype):
                        self.errors.append(TypeErrors(argument.line, argument.column, 
                            f'In call of method {node.id.value}, type {argument_type.name} of parameter {tname} does not conform to declared type {ttype.name}.'))
            else: self.errors.append(SemanticErrors(node.id.line, node.id.column, f'Method {node.id.value} called with wrong number of arguments.'))          
    ...
    def visit(self, node, scope):
        ...
        if not pred_type.conforms_to(self.bool_type):
            self.errors.append(TypeErrors(node.pred.line, node.pred.column, 
                f'pred of \'if\' does not have type Bool.'))
    ...
    def visit(self, node, scope):
        ...
        for idd, ttype, expression in node.assigs:
            if idd.value == 'self': self.errors.append(SemanticErrors(idd.line, idd.column, f'\'self\' cannot be bound in a \'let\' expression.'))
            try:
                if ttype == 'SELF_TYPE': attribute_type = self.current_type
                else: attribute_type = self.context.get_type(ttype.value)
            except KeyError:
            ...
            if expression:
                ...
                if not expression_type.conforms_to(attribute_type):
                    self.errors.append(TypeErrors(expression.line, expression.column, 
                        f'Inferred type {expression_type.name} of initialization of {idd.value} does not conform to identifier\'s declared type {ttype.value}. '))
    ...
    def visit(self, node, scope):
        ...
        for iid, ttype, expression in node.tests:
            ...
            if not node_type: node_type = variable_type
            else: node_type = node_type.join_type(variable_type)
    ...
    def visit(self, node, scope):
        if node.type.value == 'SELF_TYPE':node_type = self.current_type
        else:
            try: node_type = self.context.get_type(node.type.value)
            except KeyError:
                self.errors.append(TypeErrors(node.type.line, node.type.column, 
                    f'\'new\' used with undefined class {node.type.value}.'))
                node_type = ErrorType()
    ...
    def visit(self, node, scope):
        ...
        if not node_type.conforms_to(self.bool_type): self.errors.append(TypeErrors(node.line, node.column, f'Argument of \'not\' has type {node_type.name} instead of Bool.'))
    ...
    def visit(self, node, scope):
        ...
        if not node_type.conforms_to(self.int_type):
    ...
    def visit(self, node, scope):
        ...
        if isinstance(node, (LessNode, LequalNode)):
            node_type = self.bool_type
        else:
            node_type = self.int_type
    ...
    def visit(self, node, scope):
        ...
        if (left_type in self.basic_type or right_type in self.basic_type) and not left_type == right_type:
