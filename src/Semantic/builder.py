...
from Utils.errors import AttributeException, MethodException, SemanticError, SemanticException, TypesError
...
class TypeBuilder:
    ...
    def visit(self, node):
        ...
        if node.parent:
            try: 
                ...
                self.errors.append(SemanticError(node.parent.line, node.parent.column, error))
            except KeyError:
                self.errors.append(TypesError(node.parent.line, node.parent.column, f'Class {node.type.value} inherits from an undefined class {node.parent.value}.'))
        else:
    ...
    def visit(self, node):
        try:
            ...
            self.errors.append(TypesError(node.type.line, node.type.column, f'Class {node.type.value} of attribute {node.id.value} is undefined.'))
            attr_type = ErrorType()

        try:
            self.current_type.define_attribute(node.id.value, attr_type, node.expression)
        except AttributeException as error:
            self.errors.append(SemanticError(node.id.line, node.id.column, error))
    ...
    def visit(self, node):
        ...
        for id, type in node.parameter_list:
            try:
                if id.value == 'self':
                    self.errors.append(SemanticError(id.line, id.column, f'\'self\' cannot be the name of a formal parameter.'))
                ...
                self.errors.append(TypesError(type.line, type.column, f'Class {type.value} of formal parameter {id.value} is undefined.'))
                param_type = ErrorType()         
            
            if id.value in param_names:
                self.errors.append(SemanticError(id.line, id.column, f'Formal parameter {id.value} is multiply defined.'))
            else:
        ...
        if len(param_names) != len(param_parent_names):
            self.errors.append(SemanticError(node.id.line, node.id.column, f'Incompatible number of formal parameters in redefined method {node.id.value}'))
        else:    
            for (cid, ctype), pid, ptype in zip(node.parameter_list, param_parent_names, param_paren_types):
                if cid.value == pid:
                    if not self.context.get_type(ctype.value).conforms_to(ptype):
                        self.errors.append(SemanticError(cid.line, cid.column, f'In redefined method {node.id.value}, parameter type {ctype.value} is different from original type {ptype.name}.'))
                
            if not self.context.get_type(node.type.value).conforms_to(method.return_type):
                self.errors.append(SemanticError(node.type.line, node.type.column, f'In redefined method {node.id.value}, return type {node.type.value} is different from original return type {method.return_type.name}.'))
