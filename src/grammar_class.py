...
class Grammar:
    ...
    def compute_firsts(
        self, item: "Sentence" = None, accepts_eps=True
    ) -> Set[Terminal]:
        if self.firsts is None:
            self.firsts = {
                Sentence(s): ({s} if isinstance(s, Terminal) else set())
                for s in chain(self.terminals, self.non_terminals)
            }
            change = True
            while change:
                change = False
                for production in self.productions:
                    ...
                    change |= (
                        right not in self.firsts
                        or len(local_first - self.firsts[right]) != 0
                    )
                    change |= len(local_first - self.firsts[left_s]) != 0
                    self.firsts[left_s].update(local_first)
                    self.firsts[right] = (
                        self.firsts.get(right, local_first) | local_first
                    )
            ...
            if isinstance(current_nxt_s, NonTerminal):
                ...
                for prod in current_nxt_s.productions:
                    ...
                    if item:  # adding new loookaheads
                        item.lookaheads = tuple(
                            sorted(set(chain(item.lookaheads, lookaheads)), key=hash)
                        )
                    else:
    ...
    def lexer(self, path, ignore=" ", eof="$", eoline="\n"):
        ...
        for t in filter(
            lambda s: not (isinstance(s, Epsilon) or isinstance(s, EOF)), self.terminals
        ):  # making match table
    ...
    def parser(self, path=None):
        ...
        code = parser_content.replace(
            "def attr_decoder(self,attr, symbols_to_reduce, ast_class): pass", a_d_r
        )
