...
LOCAL_ALREADY_DEFINED = (
    '(%s, %s) - SemanticError: Variable "%s" is already defined in method "%s".'
)
ATTR_ALREADY_DEFINED = (
    '(%s, %s) - SemanticError: Attribute "%s" is already defined in ancestor class.'
)
INCOMPATIBLE_TYPES = '(%s, %s) - TypeError: Cannot convert "%s" into "%s".'
VARIABLE_NOT_DEFINED = '(%s, %s) - NameError: Variable "%s" is not defined".'
INVALID_OPERATION = (
    '(%s, %s) - TypeError: Operation is not defined between "%s" and "%s".'
)
METHOD_ARGS_UNMATCHED = (
    '(%s, %s) - SemanticError: Method "%s" arguments do not match with definition.'
)
...
class TypeChecker:
    ...
    def visit(self, node, scope, set_type=None):
        ...
        for feature in [f for f in node.features if isinstance(f, AttrDeclarationNode)]:
            ...
            if var is not None:
                self.errors.append(
                    ATTR_ALREADY_DEFINED % (feature.line_no, feature.col_no, feature.id)
                )
            else:
    ...
    def visit(self, node: AttrDeclarationNode, scope: Scope, set_type=None):
        ...
        if not return_type.conforms_to(attr_type):
            self.errors.append(
                INCOMPATIBLE_TYPES
                % (node.line_no, node.col_no, return_type.name, attr_type.name)
            )
    ...
    def visit(self, node, scope, set_type=None):
        ...
        try:
            ...
            if old_return_type.name != current_return_type.name:
                self.errors.append(
                    WRONG_SIGNATURE
                    % (node.line_no, node.col_no, node.id, ancestor_type.name)
                )
        ...
        if not expr_type.conforms_to(to_conform):
            self.errors.append(
                INCOMPATIBLE_TYPES
                % (node.line_no, node.col_no, expr_type.name, to_conform.name)
            )
    ...
    def visit(self, node, scope, set_type=None):
        ...
        if not cond_type.conforms_to(bool_type):
            self.errors.append(
                INCOMPATIBLE_TYPES
                % (node.line_no, node.col_no, cond_type.name, BasicTypes.BOOL.value)
            )
    ...
    def visit(self, node, scope, set_type=None):
        ...
        for var, typex, expr in node.var_list:
            ...
            if var == "self":
                self.errors.append(
                    SEMANTIC_ERROR
                    % (node.line_no, node.col_no, f'"self" is used as let variable')
                )
            ...
            if not expr_type.conforms_to(var_type):
                self.errors.append(
                    INCOMPATIBLE_TYPES
                    % (node.line_no, node.col_no, expr_type.name, var_type.name)
                )
    ...
    def visit(self, node, scope: Scope, set_type=None):
        ...
        if node.id == "self":
            self.errors.append(
                SEMANTIC_ERROR
                % (node.line_no, node.col_no, f'"self" variable is read-only')
            )
    ...
    def visit(self, node, scope, set_type=None):
        ...
        try:
            ...
            self.errors.append(
                ATTRIBUTE_ERROR % (node.line_no, node.col_no, error.text)
            )
            return error_type

        if not len(method.param_names) == len(node.args):
            self.errors.append(
                METHOD_ARGS_UNMATCHED % (node.line_no, node.col_no, method.name)
            )
        else:
    ...
    def visit(self, node, scope, set_type=None):
        ...
        if not left_type.conforms_to(int_type) or not right_type.conforms_to(int_type):
            self.errors.append(
                INVALID_OPERATION
                % (node.line_no, node.col_no, left_type.name, right_type.name)
            )
    ...
    def visit(self, node, scope, set_type=None):
        ...
        if not expr_type.conforms_to(int_type):
            self.errors.append(
                INCOMPATIBLE_TYPES
                % (node.line_no, node.col_no, expr_type.name, int_type.name)
            )
        node.computed_type = int_type
        return int_type
