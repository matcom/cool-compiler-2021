from . import cil
import sys
from .base_to_cil import BaseToCil
from .ast import *
sys.path.append('./semantic')
from .semantic.structures import *
from . import visitor

class CoolToCil(BaseToCil):
    @visitor.on('node')
    def visit(self, node):
        pass
    ...
    def visit(self, node, scope):
        if not node.expr is None:
            ...
            if  node.type == 'Object' and node.expr.type.name in ['Int', 'Bool', 'String']:
               value = self.box(node.expr.type.name, value)
    ...
    def visit(self, node, scope):
        ...
        for init in node.list_decl:
            if not init.expr is None:
                value = self.visit(init.expr, scopeOpen)

                if init.expr.type.name in ['Int', 'Bool', 'String'] and init.type == 'Object':
                    value = self.box(init.expr.type.name, value)
            ...
            vinfo = scopeOpen.find_variable(init.id)
            vname = self.register_local(vinfo)
            self.register_instruction(cil.AssignNode(vname, value))

        return self.visit(node.expr, scopeOpen.children[0])
    ...
    def visit(self, node, scope):
        ...
        types_ordered = self.sort_types([self.context.get_type(x.type) for x in node.list_case])
        list_label = []
        labels = dict()
        ...
        for branch, scopeBranch in zip(node.list_case, scope.children[1].children):
            vinfo = scopeBranch.find_variable(branch.id)
            xxx = self.register_local(vinfo)
            self.register_instruction(cil.LabelNode(labels[branch.type]))
            self.register_instruction(cil.AssignNode(xxx, value_expression))
           
            valueBranch = self.visit(branch.expr, scopeBranch)
    ...
    def visit(self, node, scope):
        ...
        if node.expr.type.name in ['Int', 'Bool', 'String']:
            if node.id in ['abort', 'type_name', 'copy']:
                obj = self.box(node.expr.type.name, obj)
    ...
    def visit(self, node, scope):
        msg = self.register_data(node.lex).name
        internal = self.define_internal_local()
    ...
    def visit(self, node, scope):
        ...
        if vinfo is None:
            result = self.define_internal_local()
            attrib = self.get_attr(self.current_type.name, node.lex)
