...
_lr_productions = [
  ...
  ('class_list -> def_class','class_list',1,'p_class_list','parser.py',35),
  ('class_list -> def_class class_list','class_list',2,'p_class_list','parser.py',36),
  ('def_class -> class type ocur feature_list ccur semi','def_class',6,'p_def_class','parser.py',44),
  ('def_class -> class type inherits type ocur feature_list ccur semi','def_class',8,'p_def_class','parser.py',45),
  ('feature_list -> def_attr feature_list','feature_list',2,'p_feature_list_attr','parser.py',53),
  ('feature_list -> def_func feature_list','feature_list',2,'p_feature_list_attr','parser.py',54),
  ('feature_list -> <empty>','feature_list',0,'p_feature_list_attr','parser.py',55),
  ('def_attr -> id colon type semi','def_attr',4,'p_def_attr','parser.py',79),
  ('def_attr -> id colon type assignArrow expr semi','def_attr',6,'p_def_attr','parser.py',80),
  ('def_func -> id opar arg_list cpar colon type ocur expr ccur semi','def_func',10,'p_def_func','parser.py',88),
  ('arg_list -> arg','arg_list',1,'p_arg_list','parser.py',92),
  ('arg_list -> arg comma arg_list','arg_list',3,'p_arg_list','parser.py',93),
  ('arg_list -> <empty>','arg_list',0,'p_arg_list','parser.py',94),
  ('arg -> id colon type','arg',3,'p_arg','parser.py',104),
  ('param_list -> expr_list','param_list',1,'p_param_list','parser.py',108),
  ('param_list -> <empty>','param_list',0,'p_param_list','parser.py',109),
  ('expr_list -> expr','expr_list',1,'p_expr_list','parser.py',117),
  ('expr_list -> expr comma expr_list','expr_list',3,'p_expr_list','parser.py',118),
  ('chunk -> expr semi','chunk',2,'p_chunk','parser.py',126),
  ('chunk -> expr semi chunk','chunk',3,'p_chunk','parser.py',127),
  ('expr -> id assignArrow expr','expr',3,'p_expr_assign','parser.py',135),
  ('expr -> expr dot id opar param_list cpar','expr',6,'p_expr_call','parser.py',139),
  ('expr -> expr arroba type dot id opar param_list cpar','expr',8,'p_expr_call','parser.py',140),
  ('expr -> id opar param_list cpar','expr',4,'p_expr_call','parser.py',141),
  ('expr -> if expr then expr else expr fi','expr',7,'p_expr_if','parser.py',151),
  ('expr -> while expr loop expr pool','expr',5,'p_expr_while','parser.py',155),
  ('expr -> ocur chunk ccur','expr',3,'p_expr_chunk','parser.py',159),
  ('expr -> let decl_list in expr','expr',4,'p_expr_let','parser.py',163),
  ('expr -> case expr of case_list esac','expr',5,'p_expr_case','parser.py',167),
  ('expr -> new type','expr',2,'p_expr_instantiate','parser.py',171),
  ('expr -> isvoid expr','expr',2,'p_expr_isvoid','parser.py',175),
  ('expr -> expr plus expr','expr',3,'p_expr_bin1','parser.py',179),
  ('expr -> expr minus expr','expr',3,'p_expr_bin1','parser.py',180),
  ('expr -> expr star expr','expr',3,'p_expr_bin2','parser.py',188),
  ('expr -> expr div expr','expr',3,'p_expr_bin2','parser.py',189),
  ('expr -> expr equal expr','expr',3,'p_expr_bin3','parser.py',197),
  ('expr -> expr lneq expr','expr',3,'p_expr_bin3','parser.py',198),
  ('expr -> expr leq expr','expr',3,'p_expr_bin3','parser.py',199),
  ('expr -> complement expr','expr',2,'p_expr_unary','parser.py',209),
  ('expr -> not expr','expr',2,'p_expr_unary','parser.py',210),
  ('expr -> opar expr cpar','expr',3,'p_expr_par','parser.py',218),
  ('expr -> id','expr',1,'p_expr_id','parser.py',222),
  ('expr -> number','expr',1,'p_expr_int','parser.py',226),
  ('expr -> string','expr',1,'p_expr_string','parser.py',230),
  ('expr -> true','expr',1,'p_expr_boolean','parser.py',234),
  ('expr -> false','expr',1,'p_expr_boolean','parser.py',235),
  ('decl_list -> decl','decl_list',1,'p_decl_list','parser.py',242),
  ('decl_list -> decl comma decl_list','decl_list',3,'p_decl_list','parser.py',243),
  ('decl -> id colon type','decl',3,'p_decl','parser.py',250),
  ('decl -> id colon type assignArrow expr','decl',5,'p_decl','parser.py',251),
  ('case_list -> id colon type rArrow expr semi','case_list',6,'p_case_list','parser.py',259),
  ('case_list -> id colon type rArrow expr semi case_list','case_list',7,'p_case_list','parser.py',260),
]
