...
_lr_productions = [
  ...
  ('program -> class_list','program',1,'p_program','parser.py',34),
  ('class_list -> def_class','class_list',1,'p_class_list','parser.py',38),
  ('class_list -> def_class class_list','class_list',2,'p_class_list','parser.py',39),
  ('def_class -> class type ocur feature_list ccur semi','def_class',6,'p_def_class','parser.py',47),
  ('def_class -> class type inherits type ocur feature_list ccur semi','def_class',8,'p_def_class','parser.py',48),
  ('feature_list -> def_attr feature_list','feature_list',2,'p_feature_list_attr','parser.py',56),
  ('feature_list -> def_func feature_list','feature_list',2,'p_feature_list_attr','parser.py',57),
  ('feature_list -> <empty>','feature_list',0,'p_feature_list_attr','parser.py',58),
  ('def_attr -> id colon type semi','def_attr',4,'p_def_attr','parser.py',82),
  ('def_attr -> id colon type assignArrow expr semi','def_attr',6,'p_def_attr','parser.py',83),
  ('def_func -> id opar arg_list cpar colon type ocur expr ccur semi','def_func',10,'p_def_func','parser.py',91),
  ('arg_list -> non_empty_arg_list','arg_list',1,'p_arg_list','parser.py',95),
  ('arg_list -> <empty>','arg_list',0,'p_arg_list','parser.py',96),
  ('non_empty_arg_list -> arg','non_empty_arg_list',1,'p_non_empty_arg_list','parser.py',104),
  ('non_empty_arg_list -> arg comma non_empty_arg_list','non_empty_arg_list',3,'p_non_empty_arg_list','parser.py',105),
  ('arg -> id colon type','arg',3,'p_arg','parser.py',113),
  ('param_list -> expr_list','param_list',1,'p_param_list','parser.py',117),
  ('param_list -> <empty>','param_list',0,'p_param_list','parser.py',118),
  ('expr_list -> expr','expr_list',1,'p_expr_list','parser.py',126),
  ('expr_list -> expr comma expr_list','expr_list',3,'p_expr_list','parser.py',127),
  ('chunk -> expr semi','chunk',2,'p_chunk','parser.py',135),
  ('chunk -> expr semi chunk','chunk',3,'p_chunk','parser.py',136),
  ('expr -> id assignArrow expr','expr',3,'p_expr_assign','parser.py',144),
  ('expr -> expr dot id opar param_list cpar','expr',6,'p_expr_call','parser.py',148),
  ('expr -> expr arroba type dot id opar param_list cpar','expr',8,'p_expr_call','parser.py',149),
  ('expr -> id opar param_list cpar','expr',4,'p_expr_call','parser.py',150),
  ('expr -> if expr then expr else expr fi','expr',7,'p_expr_if','parser.py',160),
  ('expr -> while expr loop expr pool','expr',5,'p_expr_while','parser.py',164),
  ('expr -> ocur chunk ccur','expr',3,'p_expr_chunk','parser.py',168),
  ('expr -> let decl_list in expr','expr',4,'p_expr_let','parser.py',172),
  ('expr -> case expr of case_list esac','expr',5,'p_expr_case','parser.py',176),
  ('expr -> new type','expr',2,'p_expr_instantiate','parser.py',180),
  ('expr -> isvoid expr','expr',2,'p_expr_isvoid','parser.py',184),
  ('expr -> expr plus expr','expr',3,'p_expr_bin1','parser.py',188),
  ('expr -> expr minus expr','expr',3,'p_expr_bin1','parser.py',189),
  ('expr -> expr star expr','expr',3,'p_expr_bin2','parser.py',197),
  ('expr -> expr div expr','expr',3,'p_expr_bin2','parser.py',198),
  ('expr -> expr equal expr','expr',3,'p_expr_bin3','parser.py',206),
  ('expr -> expr lneq expr','expr',3,'p_expr_bin3','parser.py',207),
  ('expr -> expr leq expr','expr',3,'p_expr_bin3','parser.py',208),
  ('expr -> complement expr','expr',2,'p_expr_unary','parser.py',218),
  ('expr -> not expr','expr',2,'p_expr_unary','parser.py',219),
  ('expr -> opar expr cpar','expr',3,'p_expr_par','parser.py',227),
  ('expr -> id','expr',1,'p_expr_id','parser.py',231),
  ('expr -> number','expr',1,'p_expr_int','parser.py',235),
  ('expr -> string','expr',1,'p_expr_string','parser.py',239),
  ('expr -> true','expr',1,'p_expr_boolean','parser.py',243),
  ('expr -> false','expr',1,'p_expr_boolean','parser.py',244),
  ('decl_list -> decl','decl_list',1,'p_decl_list','parser.py',251),
  ('decl_list -> decl comma decl_list','decl_list',3,'p_decl_list','parser.py',252),
  ('decl -> id colon type','decl',3,'p_decl','parser.py',259),
  ('decl -> id colon type assignArrow expr','decl',5,'p_decl','parser.py',260),
  ('case_list -> id colon type rArrow expr semi','case_list',6,'p_case_list','parser.py',268),
  ('case_list -> id colon type rArrow expr semi case_list','case_list',7,'p_case_list','parser.py',269),
]
