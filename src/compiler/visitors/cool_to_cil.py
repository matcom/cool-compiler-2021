...
class COOLToCILVisitor(BaseCOOLToCILVisitor):
    ...
    def visit(self, node, scope):
        ...
        type_node.methods = [
            (method.name, self.to_function_name(method.name, xtype.name))
            for method, xtype in self.current_type.all_methods()
        ]

        func_declarations = (
            f for f in node.features if isinstance(f, cool.FuncDeclarationNode)
        )
        ...
        attr_declarations = (
            f for f in node.features if isinstance(f, cool.AttrDeclarationNode)
        )
    ...
    def visit(self, node, scope):
        ...
        if node.expr:
            ...
            self.register_instruction(
                cil.SetAttribNode(self.vself.name, node.id, vtemp, self.current_type)
            )
    ...
    def visit(self, node, scope):
        ...
        self.register_instruction(
            cil.GetAttribNode(vcondition, scope.ret_expr, "value", "Bool")
        )
    ...
    def visit(self, node, scope):
        ...
        self.visit(node.expr, scope)
        self.register_instruction(
            cil.GetAttribNode(value, scope.ret_expr, "value", "Bool")
        )
    ...
    def visit(self, node, scope):
        ...
        self.visit(node.left, scope)
        self.register_instruction(
            cil.GetAttribNode(vleft, scope.ret_expr, "value", "Int")
        )
        self.visit(node.right, scope)
        self.register_instruction(
            cil.GetAttribNode(vright, scope.ret_expr, "value", "Int")
        )
    ...
    def visit(self, node, scope):
        ...
        self.register_runtime_error(
            equal_result,
            f"({token.row},{token.column}) - RuntimeError: Division by zero\n",
        )
    ...
    def visit(self, node, scope):
        ...
        self.visit(node.expr, scope)
        self.register_instruction(
            cil.GetAttribNode(value, scope.ret_expr, "value", "Int")
        )
    ...
    def visit(self, node, scope):
        ...
        self.register_runtime_error(
            equal_result,
            f"({token.row},{token.column}) - RuntimeError: Dispatch on void\n",
        )
    ...
    def visit(self, node, scope):
        ...
        self.register_instruction(
            cil.DynamicCallNode(type_of_node, node.id, result, self.current_type.name)
        )
    ...
    def visit(self, node, scope):
        ...
        self.register_instruction(
            cil.StaticCallNode(self.init_name(node.type), instance)
        )
        scope.ret_expr = instance
